...
static void control_led_green(MQTT_CLIENT_DATA_T *state, bool on);

// Controle do LED azul
static void control_led_blue(MQTT_CLIENT_DATA_T *state, bool on);

// Controle do LED vermelho
static void control_led_red(MQTT_CLIENT_DATA_T *state, bool on);
...
static void control_led_green(MQTT_CLIENT_DATA_T *state, bool on)
{
    // Publish state on /led/green topic and on/off led green
    const char *message = on ? "On" : "Off";
    if (on)
    ...
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/green_led/state"), message, strlen(message), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
}

// Controle do LED azul
static void control_led_blue(MQTT_CLIENT_DATA_T *state, bool on)
{
    // Publish state on /led/blue topic and on/off led blue
    const char *message = on ? "On" : "Off";
    if (on)
...
static void control_led_red(MQTT_CLIENT_DATA_T *state, bool on)
{
    // Publish state on /led/red topic and on/off led red
    const char *message = on ? "On" : "Off";
    if (on)
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T *state, bool sub)
{
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/green_led"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    else if (strcmp(basic_topic, "/green_led") == 0)
    {
        if (lwip_stricmp((const char *)state->data, "On") == 0 || strcmp((const char *)state->data, "1") == 0)
            control_led_green(state, true);
        else if (lwip_stricmp((const char *)state->data, "Off") == 0 || strcmp((const char *)state->data, "0") == 0)
            control_led_green(state, false);
    }
    else if (strcmp(basic_topic, "/blue_led") == 0)
    {
        if (lwip_stricmp((const char *)state->data, "On") == 0 || strcmp((const char *)state->data, "1") == 0)
            control_led_blue(state, true);
        else if (lwip_stricmp((const char *)state->data, "Off") == 0 || strcmp((const char *)state->data, "0") == 0)
            control_led_blue(state, false);
    }
    else if (strcmp(basic_topic, "/red_led") == 0)
    {
        if (lwip_stricmp((const char *)state->data, "On") == 0 || strcmp((const char *)state->data, "1") == 0)
            control_led_red(state, true);
        else if (lwip_stricmp((const char *)state->data, "Off") == 0 || strcmp((const char *)state->data, "0") == 0)
            control_led_red(state, false);
    }
